...
public final class ShimmerDrawable extends Drawable {
  ...
  private @Nullable CustomShimmerEffect mShimmer;

  public ShimmerDrawable() {
    mShimmerPaint.setAntiAlias(true);
  }

  public void setShimmer(@Nullable CustomShimmerEffect shimmer) {
    mShimmer = shimmer;
    if (mShimmer != null) {
  ...
  public void draw(@NonNull Canvas canvas) {
    ...
    switch (mShimmer.direction) {
      default:
      case CustomShimmerEffect.Direction.LEFT_TO_RIGHT:
        dx = offset(-translateWidth, translateWidth, animatedValue);
        dy = 0;
        break;
      case CustomShimmerEffect.Direction.RIGHT_TO_LEFT:
        dx = offset(translateWidth, -translateWidth, animatedValue);
        dy = 0f;
        break;
      case CustomShimmerEffect.Direction.TOP_TO_BOTTOM:
        dx = 0f;
        dy = offset(-translateHeight, translateHeight, animatedValue);
        break;
      case CustomShimmerEffect.Direction.BOTTOM_TO_TOP:
        dx = 0f;
        dy = offset(translateHeight, -translateHeight, animatedValue);
        break;
    }
  ...
  private void updateShader() {
    ...
    switch (mShimmer.shape) {
      default:
      case CustomShimmerEffect.Shape.LINEAR:
        boolean vertical =
            mShimmer.direction == CustomShimmerEffect.Direction.TOP_TO_BOTTOM
                || mShimmer.direction == CustomShimmerEffect.Direction.BOTTOM_TO_TOP;
        ...
        break;
      case CustomShimmerEffect.Shape.RADIAL:
        shader =
            new RadialGradient(
                width / 2f,
                height / 2f,
