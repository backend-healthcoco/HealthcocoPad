...
public class ShimmerFrameLayout extends FrameLayout {
    ...
    private void init(Context context, @Nullable AttributeSet attrs) {
        ...
        if (attrs == null) {
            setShimmer(new CustomShimmerEffect.AlphaHighlightBuilder().build());
            return;
        }

        TypedArray a = context.obtainStyledAttributes(attrs, R.styleable.ShimmerFrameLayout, 0, 0);
        try {
            CustomShimmerEffect.Builder shimmerBuilder =
                    a.hasValue(R.styleable.ShimmerFrameLayout_shimmer_colored)
                            ...
                            ? new CustomShimmerEffect.ColorHighlightBuilder()
                            : new CustomShimmerEffect.AlphaHighlightBuilder();
    ...
    public ShimmerFrameLayout setShimmer(@Nullable CustomShimmerEffect shimmer) {
